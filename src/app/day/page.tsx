  ...
  const eventsByTime = Object.fromEntries(
    timeSlots.map(
      slot =>
        [
          slot,
          sampleEvents.filter(ev => ev.time === slot)
        ] as [string, typeof sampleEvents]
    )
  );
    ...
    <div className="w-full max-w-3xl mx-auto px-2 py-6 flex flex-col gap-8 font-raleway">
      ...
      <div className="bg-white rounded-xl border border-black shadow-[4px_4px_0_0_black] flex flex-col gap-0 p-0 mb-2 overflow-x-auto">
        <div className="flex flex-col divide-y">
          {timeSlots.map(slot => (
            <div key={slot} className="flex items-center px-4 py-3 min-h-[32px]">
              <div className="text-xs sm:text-sm font-mono w-[54px] flex-shrink-0 text-gray-400">
                {slot}
              </div>
